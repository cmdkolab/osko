WebOS.registerApp({
    id: "syslog",
    get name() { return globalThis.I18n.t('syslog.title'); },
    icon: "📜",
    version: "4.9.0",
    manifest: {
        get name() { return globalThis.I18n.t('syslog.title'); },
                ...
                <div class="syslog-app">
                    <div class="syslog-toolbar">
                        <div class="toolbar-left">
                            <button class="sys-btn filter-btn active" data-filter="ALL">${globalThis.I18n.t('syslog.filter_all')}</button>
                            ...
                            <select class="source-select">
                                <option value="ALL">${globalThis.I18n.t('syslog.source_all')}</option>
                            </select>
                        </div>
                        <div class="toolbar-right">
                            <input type="text" class="syslog-search" placeholder="${globalThis.I18n.t('syslog.search')}">
                            ...
                            <button class="sys-btn clear-btn danger" title="${globalThis.I18n.t('syslog.confirm_clear')}">🗑️</button>
                        </div>
                    </div>
                    <div class="syslog-viewer"></div>
                    <div class="syslog-status">
                         <span class="log-info">v${this.version}</span>
                         <span class="log-count">0 ${globalThis.I18n.t('syslog.lines')}</span>
                    </div>
                </div>
            `;
    ...
    setupListeners(container) {
        ...
        container.querySelector('.save-btn').onclick = async () => {
            ...
            if (logs) {
                ...
                api.notifications.show({ title: globalThis.I18n.t('syslog.app_title'), message: globalThis.I18n.t('syslog.saved_to', path) });
            }
        };
        container.querySelector('.clear-btn').onclick = () => {
            api.ui.confirm(globalThis.I18n.t('syslog.confirm_clear'), async (ok) => {
                if (ok) {
    ...
    async refresh() {
        ...
        try {
            let logs = null;
            if (globalThis.SysLog) logs = globalThis.SysLog.getBuffer();
            if (!logs) {
                try { logs = await this.api.fs.read('/var/log/syslog'); } catch(e) { logs = null; }
            }
            if (!logs) {
                this.viewer.innerHTML = `<div class="log-empty">${globalThis.I18n.t('syslog.no_logs')}</div>`;
            ...
            const renderBatch = () => {
                ...
                for (let i = idx; i < end; i++) {
                    ...
                    entry.onclick = () => { this.api.system.setClipboard(p.raw); this.api.notifications.show({ title: globalThis.I18n.t('syslog.app_title'), message: globalThis.I18n.t('syslog.copied_to_clipboard') }); };
                ...
                if (idx < displayLines.length) {
                    ...
                    if (status) status.innerText = `${totalLines} ${globalThis.I18n.t('syslog.lines')}`;
